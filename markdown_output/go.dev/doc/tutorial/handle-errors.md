...
3.  [Return and handle an error](/doc/tutorial/handle-errors)
...
1.  In greetings/greetings.go, add the code highlighted below.
    ...
    *   Change the function so that it returns two values: a `string` and an `error`. Your caller will check the second value to see if an error occurred. (Any Go function can return multiple values. For more, see [Effective Go](/doc/effective_go.html#multiple-returns).)
...
[< Call your code from another module](/doc/tutorial/call-module-code.html) [Return a random greeting >](/doc/tutorial/random-greeting.html)