Are you an LLM? You can read better optimized documentation at /guide/best-practices/accessibility.md for this page in Markdown format
...
When this pattern is used inside a reusable component, generate the IDs with [`useId()`](/api/composition-api-helpers#useid) instead of hard-coding them. This keeps each component instance's `id` values unique while still linking the visible text to the form control:
