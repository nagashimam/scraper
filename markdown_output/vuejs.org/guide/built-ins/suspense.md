Are you an LLM? You can read better optimized documentation at /guide/built-ins/suspense.md for this page in Markdown format
...
It is common to want to use `<Suspense>` in combination with the [`<Transition>`](/guide/built-ins/transition) and [`<KeepAlive>`](/guide/built-ins/keep-alive) components. The nesting order of these components is important to get them all working correctly.
