...
**Note:** An empty `alt` attribute is not the same as a missing `alt` attribute. Always provide an `alt` attribute even if an image is presentational and the `alt` attribute has no content.
...
SVG includes how to draw the shapes and lines, at any pixel density or scale or zoom.

False

Unlike `.png` or `.jpg`, SVG has no need for `srcset` or a `<picture>` element.

SVG code that's directly in the HTML has which advantages?

Styleable from CSS

Match SVG icon shapes to buttons and brand colors.
